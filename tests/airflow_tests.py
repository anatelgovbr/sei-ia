...
def run_command(container, command: str) -> str:
    ...
    if result.exit_code != 0:
        raise RuntimeError(
            f"Comando Airflow {command!r} falhou com exit {result.exit_code}: {output}"
        )
    return output
...
def get_airflow_dag_import_error(
    container, error_airflow_lines: list[str]
) -> list[str]:
...
def get_dags_runs(
    container, airflow_dags_df: pd.DataFrame, dag_filename_error: list[str]
) -> pd.DataFrame:
    runs_data = []
