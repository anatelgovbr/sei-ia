                        ...
                        for old_file_vars in old_vars.values():
                            if old_key in old_file_vars:
                                ...
                                updated_lines.append(
                                    f"{export_prefix}{key}={new_value}\n"
                                )
                                break
                        else:
                            updated_lines.append(
                                line
                            )  # Mantém a linha original se não houver atualização
                        break
                else:
                    updated_lines.append(
                        line
                    )  # Mantém a linha original se não estiver no mapeamento
            else:
...
old_dir = input(
    "Digite o caminho para o diretório dos arquivos antigos (default_old.env, prod_old.env, security_old.env): "
)
