...
> O model type `mini` é mapeado para o modelo atual configurado no Azure OpenAI.
...
| Arquivo | Função |
|---------|--------|
| `searx_crawl_tool.py` | Busca no SearXNG e coleta páginas com fastCRW/Byparr |
| `web_research_agent.py` | Pesquisa rasa com persistência das fontes na sessão |
| `deep_research_agent.py` | Pesquisa profunda com extração e síntese internas |
...
class UserState(TypedDict):
    ...
    model_type: Literal["mini", "standard", "nano"]
    temperature: float
