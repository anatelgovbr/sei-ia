...
def generate_multiple_questions(user_state: UserState) -> list[str]:
    ...
    try:
        agent_tag = user_state.get("agent_tag", "principal")
        logger.debug(f"Usando papel de agente {agent_tag} para gerar perguntas")

        llm = get_llm_model(agent_tag=agent_tag)
