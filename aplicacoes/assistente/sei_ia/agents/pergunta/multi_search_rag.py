...
def _normalize_chunk(doc_id: str, chunk) -> dict:
    if isinstance(chunk, dict):
        chunk["id_documento"] = doc_id
        return chunk
    return {
        "id_documento": doc_id,
        "text": str(chunk.get("text", "") if hasattr(chunk, "get") else chunk),
...
async def search_with_multiple_questions(  # noqa: C901, PLR0912  # NOSONAR
...
async def search_single_question(
    ...
    try:
        ...
        _, document_chunks_dict = await similarity_query(
        ...
        return chunks_list  # noqa: TRY300
