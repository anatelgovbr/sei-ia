...
_NAO_DISPONIVEL = "Não disponível"


def build_prompt_with_complete_documents(  # NOSONAR
    ...
    id_to_formatted = {}
    selected_procs = []
    for score, _content, doc_id, doc_id_formatado, _meta in documents_with_scores:
        id_to_formatted[doc_id] = doc_id_formatado
        for proc in user_state.get("id_procedimentos", []):
            matched_docs = [
                doc
                for doc in (proc.id_documentos or [])
                if getattr(doc, "id_documento", None) == doc_id
            ]
            if matched_docs:
                selected_procs.append(
                    proc.model_copy(update={"id_documentos": matched_docs})
                )
                break
        logger.debug(
    ...
    {format_procedures_context(selected_procs)}
    [/DOCUMENTOS RELEVANTES]
    ...
    logger.info(
        f"Prompt montado com {len(documents_with_scores)} documentos completos ordenados por relevância"
    )
...
def build_prompt_with_grouped_chunks(chunks: list[dict], user_state: UserState) -> str:  # noqa: PLR0912  # NOSONAR
    """
    ...
    for doc_id, doc_chunks in chunks_by_doc.items():
        ...
        doc_metadata_str = _NAO_DISPONIVEL
        proc_metadata_str = _NAO_DISPONIVEL
        ...
        for proc in user_state["id_procedimentos"]:
            for doc in proc.id_documentos:
                if doc.id_documento == doc_id:
                    ...
                    break
            if doc_metadata_str != _NAO_DISPONIVEL:
                break
...
def _format_metadata_dict(metadata: dict) -> str:
    ...
    return ", ".join(formatted_items) if formatted_items else _NAO_DISPONIVEL
