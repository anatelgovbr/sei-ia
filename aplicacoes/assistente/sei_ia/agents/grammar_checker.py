...
def make_prompt_with_doc_grammar_correction(
    ...
    for item_proc in user_state["id_procedimentos"]:
        ...
        for item_doc in item_proc.id_documentos:
            proc_info = (
                f" | Processo: {protocolo_processo}" if protocolo_processo else ""
            )
            doc_header = f"# Documento {item_doc.id_documento_formatado}{proc_info}\n"
            doc_block = INTERMEDIATE_COMPLETATION_WITH_DOC.format(
                id_documento_formatado=item_doc.id_documento_formatado,
                protocolo_processo=protocolo_processo,
                doc=item_doc.content,
            )
            content_documents += doc_header + doc_block
