...
from sei_extraction.text import get_file_extension
...
class MetadataDocument(BaseModel):
    ...
    formato_arquivo: str
    dta_inclusao: datetime | str | None = None
    ...
    def parse_dta_inclusao(cls, value):
        """Normaliza `dta_inclusao` para a string date-only "YYYY-MM-DD".

        Invariante única da metadata exposta ao LLM. A lib `sei_api` devolve um
        `pd.Timestamp` (que os call sites estringam para "YYYY-MM-DD HH:MM:SS"
        antes de chegar aqui), então o validador aceita tanto `datetime` quanto
        a string com horário e corta o componente de hora. Mantém o valor
        original quando não há data parseável (e.g. string vazia ou "N/A").

        Args:
            value: O valor do campo `dta_inclusao`.

        Returns:
            A data no formato "YYYY-MM-DD", ou o valor original quando não parseável.
        """
...
async def get_proc_id_from_doc_id(id_documento: str) -> str:
    ...
    data_frame = await asyncio.to_thread(
        sei_client.internal_docs_from_process_api, id_documento
    )
...
async def get_type_doc_from_id(id_documento: str) -> tuple[bool, str, str, str]:
    ...
    """
    df_types = await asyncio.to_thread(
        sei_client.internal_docs_from_process_api, id_documento
    )
    l_df = len(df_types)
    if l_df == 0:
...
async def fetch_procedimentos_metadata_batch(
    ...
    try:
        ...
        data_frame = await sei_client.md_ia_consulta_processo_batch(id_procedimentos)
        ...
        logger.exception("Erro ao buscar metadados em lote de procedimentos")
        return {}
...
async def fetch_documentos_metadata_batch(id_documentos: list[str]) -> dict[str, dict]:
    ...
    try:
        # Awaita o sibling async direto. O batch sync chama run_async internamente
        # em multi-chunk; embrulhá-lo em to_thread criaria um duplo-bridge
        # (thread → novo loop). A consulta async chunka por config.chunk_size e
        # espelha o mesmo conjunto de colunas.
        data_frame = await sei_client.md_ia_consulta_documento_async(
            ",".join(str(d) for d in id_documentos)
        )
        ...
        logger.exception("Erro ao buscar metadados em lote de documentos")
        return {}
