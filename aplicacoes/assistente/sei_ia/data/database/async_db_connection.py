...
_ERR_RETRIEVE_RECORD = "Failed to retrieve the record."
_ERR_ASYNC_ENGINE_UNAVAILABLE = (
    "AsyncEngine não está disponível. Verifique a inicialização do banco."
)
...
class AsyncDbConnector:
    ...
    def require_pool(self) -> Pool:
        """Return the pool opened by the application lifespan."""
        if self.pool is None:
            msg = "Pool do banco não foi inicializado pelo lifespan da aplicação."
            raise AsyncDbQueryError(msg)
        return self.pool
    ...
    async def close(self) -> None:
        ...
        if errors:
            raise ExceptionGroup("Failed to close database resources.", errors)
    ...
    def get(self, model: Table, primary_key: int | str) -> object | None:
        ...
        try:
            ...
            session.rollback()
            logger.exception(_ERR_RETRIEVE_RECORD)
            msg = _ERR_RETRIEVE_RECORD
            raise SQLAlchemyError(msg) from e
        finally:
            session.close()
    ...
    async def execute_query_native_async(
        ...
        if not self.async_engine:
            msg = _ERR_ASYNC_ENGINE_UNAVAILABLE
            logger.error(msg)
