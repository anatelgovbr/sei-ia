...
def _get_non_cacheable_id(doc) -> str | None:
    ...
    if isinstance(doc, dict):
        ...
        doc_dict = {}
    if doc_dict.get("sin_armazena_cache", "S") == "N":
        id_doc = doc_dict.get("id_documento")
        return str(id_doc) if id_doc else None
    return None
...
def _extract_non_cacheable_ids(user_state: UserState) -> list[str]:
    ...
    for proc in id_procedimentos:
        ...
        for doc in id_documentos:
            doc_id = _get_non_cacheable_id(doc)
            if doc_id:
                ids.append(doc_id)
            ...
            async with conn.begin():
                _tref = ".".join(
                    [
                        ...
                        settings.EMBEDDINGS_TABLE_NAME,
                    ]
                )
                _sql = "DELETE FROM {t} WHERE id_documento = ANY(:ids) RETURNING id_documento"
                query = text(_sql.replace("{t}", _tref))
