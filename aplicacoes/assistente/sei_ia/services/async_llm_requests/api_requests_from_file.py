...
async def api_requests_from_file(  # noqa: C901, PLR0915  # NOSONAR
    requests_filepath: str,
    save_filepath: str,
    api_endpoint: str,
    ...
    with Path(requests_filepath).open() as file:  # NOSONAR
            ...
            while True:
                ...
                if next_request:
                    ...
                    if (
                        ...
                        # call API
                        asyncio.create_task(  # noqa: RUF006  # NOSONAR
...
class APIRequest:
    ...
    async def _dispatch_api_call(
        self, api_endpoint: str, llm_async_client: AsyncOpenAI
    ):
        ...
        if "completions" in api_endpoint:
    ...
    async def call_api(
        ...
        try:
            response_data = await self._dispatch_api_call(
                api_endpoint, llm_async_client
            )
...
def append_to_jsonl(data: dict | list, filename: str) -> None:
...
def num_tokens_consumed_from_request(  # noqa: C901  # NOSONAR
    request_json: dict,
    api_endpoint: str,
    token_encoding: tiktoken.core.Encoding,
) -> int:
    ...
    if api_endpoint == "embeddings":
        ensure_embedding_input(
            request_json.get("input_texts"),
            first_document_id(request_json),
        )
...
def task_id_generator_function() -> Iterator[int]:
