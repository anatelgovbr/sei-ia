...
from sei_ia.configs.settings_config import LITELLM_EMBEDDING_ALIAS, settings
...
class APIRequest:
    ...
    async def call_api(  # noqa: C901, PLR0915, RUF100  # NOSONAR
        self,
        ...
        if "embeddings" in api_endpoint:
            ensure_embedding_input(
                self.request_json.get("input_texts"),
                first_document_id(self.request_json),
            )
        ...
        try:
            ...
            if "embeddings" in api_endpoint:
                response = await llm_client.embeddings.create(
                    ...
                    model=self.request_json.get("model", LITELLM_EMBEDDING_ALIAS),
                    extra_body={
                        "tags": self.request_json.get("tags", ["agents:embedding"])
                    },
                )
                ...
                raise ValueError("Endpoint não suportado: " + api_endpoint)  # noqa: RUF100, TRY301
...
async def process_requests(
    ...
    Args:
        ...
        db: Conector aberto e pertencente ao lifespan da aplicação.
        encoding_name: Nome da codificação de tokens a ser utilizada.
    """
    ...
    async with httpx.AsyncClient(timeout=http_timeout) as session:
        ...
        with Path(requests_filepath).open("r") as file:  # NOSONAR
...
def calculate_token_usage(
    ...
    if "embeddings" in endpoint:
        ...
        ensure_embedding_input(inputs, first_document_id(request_json))
