...
from langchain_text_splitters import RecursiveCharacterTextSplitter as RecursiveSplitter
...
def split_chunks(
    ...
    *,
    ...
    Returns:
        tuple: (chunks, positions) onde positions é lista vazia se return_positions=False.
    """
    ...
    return chunks, positions  # NOSONAR


def get_document_state_by_id(
...
def process_document(item_doc: ItemDocumentRequest, req_filepath: Path) -> None:
    """Processa um documento, gerando chunks e adicionando ao pool de embedding.

    Content arrives already cleaned from the extraction boundary
    (``fetch_document_text`` applies ``clean_text``/``html_to_markdown`` internally),
    so no re-clean is needed here.

    Args:
        item_doc (ItemDocumentRequest): Documento a ser processado.
        req_filepath (Path): Caminho do arquivo temporário para o pool.
    """
    content = item_doc.content or ""
    if not content.strip():
        raise DocumentContentNotExtractableException(item_doc.id_documento)
...
def read_embedding_results(save_filepath: Path) -> list:
    ...
    result_file = []
    try:
        with save_filepath.open("r") as file:  # NOSONAR
...
async def indexing_embeddings(
    ...
    try:
        for item_doc_trigger in list_to_trigger:
            item_doc_selected = get_document_state_by_id(item_doc_trigger, user_state)
            if item_doc_selected:
                try:
                    process_document(item_doc_selected, req_filepath)
                except (
                    HTTPException204,
                    HTTPException404,
                    HTTPException409,
        ...
        result_file = read_embedding_results(save_filepath)
        await save_embeddings_to_db(result_file)
    finally:
        req_filepath.unlink(missing_ok=True)
        save_filepath.unlink(missing_ok=True)
