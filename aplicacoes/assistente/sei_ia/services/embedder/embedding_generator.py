...
class EmbeddingGenerator:
    ...
    def append_pool_file(self, pool_input: InputPoolEmbd, req_filepath: Path) -> None:
        ...
        if not pool_input.input_texts:
            raise DocumentContentNotExtractableException(pool_input.doc_id)
        if not (
            len(pool_input.input_texts)
            == len(pool_input.chunk_ids)
            == len(pool_input.positions)
        ):
            raise EmbeddingInputException(
                detail=(
                    f"Os chunks do documento {pool_input.doc_id} estão desalinhados; "
                    "o pool de embeddings não foi gravado."
                ),
                document_id=pool_input.doc_id,
            )
