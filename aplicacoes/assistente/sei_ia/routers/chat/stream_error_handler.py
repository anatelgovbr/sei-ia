...
from sei_ia.routers.chat import _trace_id_from_span, _update_langfuse_trace
...
def handle_http_exception(exc: HTTPException, span: Any) -> dict:
    ...
    logger.error(
        f"HTTPException {exc.status_code} no streaming: {exc.detail} "
        f"[trace_id={_trace_id_from_span(span)}]"
    )
...
def handle_openai_bad_request(exc: openai.BadRequestError, span: Any) -> dict:
    logger.error(
        f"openai.BadRequestError no streaming: {exc} "
        f"[trace_id={_trace_id_from_span(span)}]"
    )
...
def handle_openai_internal_server_error(
    ...
    logger.exception(
        f"openai.InternalServerError no streaming: {exc} "
        f"[trace_id={_trace_id_from_span(span)}]"
    )
    _update_langfuse_trace(
        span,
...
def handle_rate_limit(exc: openai.RateLimitError, span: Any) -> dict:
    logger.error(
        f"openai.RateLimitError no streaming: {exc} "
        f"[trace_id={_trace_id_from_span(span)}]"
    )
    _update_langfuse_trace(
        span,
...
def handle_timeout(
    ...
    logger.error(
        f"Timeout com LLM proxy no streaming: {type(exc).__name__}: {exc} "
        f"[trace_id={_trace_id_from_span(span)}]"
    )
    _update_langfuse_trace(
        span,
...
def handle_chat_error(exc: ChatError, span: Any) -> dict:
    ...
    logger.error(
        f"ChatError no streaming: [{exc.status_code}] {exc.detail} "
        f"[trace_id={_trace_id_from_span(span)}]"
    )
    _update_langfuse_trace(
        span,
...
def handle_unhandled_exception(exc: Exception, span: Any) -> dict:
    ...
    logger.exception(
        f"Exceção não tratada no streaming: {type(exc).__name__} "
        f"[trace_id={_trace_id_from_span(span)}]"
    )
    status_code = getattr(exc, "status_code", 500)
    detail = getattr(exc, "detail", None) or f"Erro interno: {exc!s}"
    _update_langfuse_trace(
        span,
        output={
            "error": str(exc),
