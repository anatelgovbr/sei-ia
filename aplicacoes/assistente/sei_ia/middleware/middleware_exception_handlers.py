...
async def sqlalchemy_exception_handler(  # NOSONAR
    request: Request, exc: SQLAlchemyError
) -> JSONResponse:
    """SQLAlchemy exception handler."""
    trace_id = getattr(request.state, "trace_id", None)
    logger.exception(f"Erro no banco de dados [trace_id={trace_id}]")
...
async def http_exception_handler(  # NOSONAR
    ...
    """
    trace_id = getattr(request.state, "trace_id", None)
    logger.exception(f"Capturou um erro na request. [trace_id={trace_id}]")
    state: dict = request.state._state  # noqa: RUF100, SLF001
    ...
    # payload: dict[str, Any] = await _extract_request_payload(request)  # noqa: ERA001
