"""
Testes unitários para funções utilitárias de sei_ia/routers/chat/__init__.py.

Cobre:
1. _build_langfuse_tags — caminho de exceção (state.get() lança exceção)
2. _build_langfuse_tags — caminhos normais (com e sem tags)
3. create_user_state — body decode error (state.body não é bytes decodificáveis UTF-8)
"""

from unittest.mock import MagicMock, patch

import pytest

# ---------------------------------------------------------------------------
# 1. _build_langfuse_tags
# ---------------------------------------------------------------------------


def test_build_langfuse_tags_excecao_retorna_lista_vazia():
    """
    Se state.get() lança exceção, _build_langfuse_tags deve capturar e retornar [].
    """
    from sei_ia.routers.chat import _build_langfuse_tags

    bad_state = MagicMock()
    bad_state.get.side_effect = RuntimeError("state corrompido")

    tags = _build_langfuse_tags(bad_state)

    assert tags == []


def test_build_langfuse_tags_sem_campos_retorna_lista_vazia():
    """State sem campos relevantes deve retornar lista vazia."""
    from sei_ia.routers.chat import _build_langfuse_tags

    state = {
        "intent": None,
        "use_websearch": False,
        "use_thinking": False,
        "doc_rag": False,
        "rag_method": None,
    }

    tags = _build_langfuse_tags(state)

    assert tags == []


def test_build_langfuse_tags_com_intent():
    """State com intent deve incluir tag 'intent:<valor>'."""
    from sei_ia.routers.chat import _build_langfuse_tags

    state = {
        "intent": "pesquisa",
        "use_websearch": False,
        "use_thinking": False,
        "doc_rag": False,
        "rag_method": None,
    }
    tags = _build_langfuse_tags(state)

    assert "intent:pesquisa" in tags


def test_build_langfuse_tags_com_websearch():
    """State com use_websearch=True deve incluir tag 'websearch'."""
    from sei_ia.routers.chat import _build_langfuse_tags

    state = {
        "intent": None,
        "use_websearch": True,
        "use_thinking": False,
        "doc_rag": False,
        "rag_method": None,
    }
    tags = _build_langfuse_tags(state)

    assert "websearch" in tags


def test_build_langfuse_tags_com_thinking():
    """State com use_thinking=True deve incluir tag 'thinking'."""
    from sei_ia.routers.chat import _build_langfuse_tags

    state = {
        "intent": None,
        "use_websearch": False,
        "use_thinking": True,
        "doc_rag": False,
        "rag_method": None,
    }
    tags = _build_langfuse_tags(state)

    assert "thinking" in tags


def test_build_langfuse_tags_com_rag_method():
    """State com rag_method deve incluir tag 'rag_method:<valor>'."""
    from sei_ia.routers.chat import _build_langfuse_tags

    state = {
        "intent": None,
        "use_websearch": False,
        "use_thinking": False,
        "doc_rag": True,
        "rag_method": "hybrid",
    }
    tags = _build_langfuse_tags(state)

    assert "rag" in tags
    assert "rag_method:hybrid" in tags


# ---------------------------------------------------------------------------
# 2. create_user_state — body decode error path
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_create_user_state_body_decode_error():
    """
    Quando state.body não pode ser decodificado como UTF-8,
    create_user_state deve usar str(body) como fallback e não lançar exceção.
    """
    from sei_ia.data.pydantic_models import ChatRequest
    from sei_ia.routers.chat import create_user_state

    mock_request_starlette = MagicMock()
    mock_request_starlette.state.body = b"\xff\xfe bytes inv\xe1lidos"  # não é UTF-8

    chat_request = ChatRequest(id_usuario=1, id_topico=0, text="Olá")

    model_data = {
        "agent_tag": "classificador",
        "max_ctx_len": 128_000,
        "model_name": "gpt-4o-mini",
    }

    mock_model_params = {
        "model": "gpt-4o-mini",
        "max_ctx_len": 128_000,
        "model_name": "gpt-4o-mini",
    }

    with patch("sei_ia.routers.chat.get_model_config", return_value=mock_model_params):
        # Deve funcionar sem lançar exceção mesmo com bytes inválidos
        user_state = await create_user_state(
            chat_request, mock_request_starlette, model_data
        )

    assert user_state is not None
    # O campo original_request_body deve ser uma string (fallback para str(bytes))
    assert isinstance(user_state.get("original_request_body"), str)
    assert "temperature" not in user_state


@pytest.mark.asyncio
async def test_create_user_state_propaga_model_override():
    """create_user_state deve copiar o `model` (override) do request para o
    UserState, depois de validá-lo contra o catálogo ao vivo do proxy."""
    from sei_ia.data.pydantic_models import ChatRequest
    from sei_ia.routers.chat import create_user_state

    mock_request_starlette = MagicMock()
    mock_request_starlette.state.body = b"{}"

    chat_request = ChatRequest(
        id_usuario=1, id_topico=0, text="Olá", model="openai/seiia-ds-gemini-pro"
    )

    model_data = {"agent_tag": "principal"}
    mock_model_params = {
        "model": "openai/seiia-ds-gemini-pro",
        "max_ctx_len": 128_000,
        "max_output_tokens": 16_000,
        "model_name": "openai/seiia-ds-gemini-pro",
    }

    with (
        patch("sei_ia.routers.chat.get_model_config", return_value=mock_model_params),
        patch(
            "sei_ia.routers.chat.validate_model_override", return_value=None
        ) as mock_validate,
    ):
        user_state = await create_user_state(
            chat_request, mock_request_starlette, model_data
        )

    mock_validate.assert_awaited_once_with("openai/seiia-ds-gemini-pro")
    assert user_state["model_override"] == "openai/seiia-ds-gemini-pro"


@pytest.mark.asyncio
async def test_create_user_state_model_override_invalido_vira_422():
    """`model` (override) não liberado pro papel principal vira HTTPException
    422, nunca passa silencioso."""
    from fastapi import HTTPException

    from sei_ia.data.pydantic_models import ChatRequest
    from sei_ia.routers.chat import create_user_state

    mock_request_starlette = MagicMock()
    mock_request_starlette.state.body = b"{}"

    chat_request = ChatRequest(
        id_usuario=1, id_topico=0, text="Olá", model="modelo-arbitrario"
    )
    model_data = {"agent_tag": "principal"}

    with (
        patch(
            "sei_ia.routers.chat.validate_model_override",
            side_effect=ValueError("model='modelo-arbitrario' não está liberado"),
        ),
        pytest.raises(HTTPException) as exc_info,
    ):
        await create_user_state(chat_request, mock_request_starlette, model_data)

    assert exc_info.value.status_code == 422
    assert "modelo-arbitrario" in exc_info.value.detail


@pytest.mark.asyncio
async def test_create_user_state_propaga_reasoning_effort_valido():
    """reasoning_effort validado com sucesso contra o catálogo do proxy vira
    UserState["reasoning_effort"]."""
    from sei_ia.data.pydantic_models import ChatRequest
    from sei_ia.routers.chat import create_user_state

    mock_request_starlette = MagicMock()
    mock_request_starlette.state.body = b"{}"

    chat_request = ChatRequest(
        id_usuario=1, id_topico=0, text="Olá", reasoning_effort="medium"
    )
    model_data = {"agent_tag": "principal"}
    mock_model_params = {
        "model": "openai/seiia-ds-gpt-terra",
        "max_ctx_len": 128_000,
        "max_output_tokens": 16_000,
        "model_name": "openai/seiia-ds-gpt-terra",
    }

    with (
        patch("sei_ia.routers.chat.get_model_config", return_value=mock_model_params),
        patch(
            "sei_ia.routers.chat.validate_reasoning_effort", return_value=None
        ) as mock_validate,
    ):
        user_state = await create_user_state(
            chat_request, mock_request_starlette, model_data
        )

    mock_validate.assert_awaited_once_with("openai/seiia-ds-gpt-terra", "medium")
    assert user_state["reasoning_effort"] == "medium"


@pytest.mark.asyncio
async def test_create_user_state_reasoning_effort_invalido_vira_422():
    """reasoning_effort não suportado pelo modelo alvo vira HTTPException 422,
    nunca passa silencioso."""
    from fastapi import HTTPException

    from sei_ia.data.pydantic_models import ChatRequest
    from sei_ia.routers.chat import create_user_state

    mock_request_starlette = MagicMock()
    mock_request_starlette.state.body = b"{}"

    chat_request = ChatRequest(
        id_usuario=1, id_topico=0, text="Olá", reasoning_effort="high"
    )
    model_data = {"agent_tag": "principal"}
    mock_model_params = {
        "model": "openai/seiia-ds-gpt-terra",
        "max_ctx_len": 128_000,
        "max_output_tokens": 16_000,
        "model_name": "openai/seiia-ds-gpt-terra",
    }

    with (
        patch("sei_ia.routers.chat.get_model_config", return_value=mock_model_params),
        patch(
            "sei_ia.routers.chat.validate_reasoning_effort",
            side_effect=ValueError("reasoning_effort='high' não é suportado"),
        ),
        pytest.raises(HTTPException) as exc_info,
    ):
        await create_user_state(chat_request, mock_request_starlette, model_data)

    assert exc_info.value.status_code == 422
    assert "high" in exc_info.value.detail
