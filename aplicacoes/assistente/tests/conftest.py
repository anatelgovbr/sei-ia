...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers",
        "no_stt_mode_patch: pula o autouse que fixa o modo STT em 'transcriptions' "
        "(usado por testes que exercitam _resolve_stt_mode diretamente)",
    )
    config.addinivalue_line(
        "markers",
        "no_audio_conversion_patch: pula o autouse que fakeia _transcode_to_ogg/"
        "_get_audio_duration_sec (usado por testes que exercitam essas funções "
        "diretamente via mocks de subprocess.run)",
    )
...
def mock_openai_api(monkeypatch):
    ...
    def mock_chat_completion(*args, **kwargs):  # noqa: ARG001
...
def mock_database(monkeypatch):
    ...
    def mock_select_async(*args, **kwargs):  # noqa: ARG001
...
def mock_embedding_service(monkeypatch):
    ...
    def mock_generate(*args, **kwargs):  # noqa: ARG001
...
async def configure_settings(
    db_config,
    redis_config,
    postgres_container,  # noqa: ARG001
    redis_container,  # noqa: ARG001
):
    """
...
async def db_connection(configure_settings, db_config):  # noqa: ARG001
    """
...
@pytest_asyncio.fixture(scope="function")
async def insert_test_data(clean_test_data, db_connection):  # noqa: ARG001
    """
