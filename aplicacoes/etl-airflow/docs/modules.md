...
| `providers/litellm.py` | Implementação LiteLLM Proxy (gateway para Azure OpenAI ou Google Vertex AI) |

!!! note "Provider Recomendado"
    O `LiteLLMEmbeddingProvider` é o provider padrão e recomendado.
    Utiliza o LiteLLM Proxy como gateway para Azure OpenAI ou Google
    Vertex AI — o provider real é definido na configuração do proxy
    (`litellm_config.yaml`), não neste código. Ver
    [ETL de Embeddings](etl/embeddings.md) para detalhes de configuração
    por provider.
...
| `LITELLM_EMBEDDING_MODEL_NAME` | Alias público usado na requisição (`embedding`) |
| `EMBEDDING_BASE_MODEL` | Pin físico usado na tokenização e no nome da tabela |
| `EMBEDDING_MODEL` | Alias legado de execução avulsa |
| `REDIS_URI` | URI do Redis |
