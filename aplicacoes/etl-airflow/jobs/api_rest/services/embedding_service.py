    ...
    *,
    ...
    if embedding_generator.provider.tokenizer_type == "tiktoken":
        text_splitter = RecursiveSplitter.from_tiktoken_encoder(
            model_name=embedding_generator.provider.tiktoken_model_name,
            chunk_size=chunk_size,
            chunk_overlap=chunk_overlap,
            separators=separators,
        )
...
def save_embeddings_to_db(result_file: list) -> None:
...
async def generate_embeddings_for_documents(id_documentos: list[str]) -> dict:
    ...
    try:
        ...
        for doc_id in docs_to_process:
            try:
                ...
                if content:
                    ...
                    logger.warning(
                        f"Documento {doc_id} sem conteúdo após extração"
                        " — será marcado como vetorizado para sair da fila de retentativas"
                    )
                    no_content_ids.append(doc_id)
        ...
        for doc_id, content in contents_map.items():
            process_document_for_embedding(doc_id, content, req_filepath)

        # Gerar embeddings (implementação simplificada - na prática usaria async pool)
        logger.info("Gerando embeddings...")
        result_file = generate_embeddings_from_pool(req_filepath, save_filepath)

        # Salvar no banco
        save_embeddings_to_db(result_file)
...
def generate_embeddings_from_pool(req_filepath: Path, save_filepath: Path) -> list:
