...
class ProcessFromSEI:
    ...
    def get_id_documents_allowed(processes_str: str) -> str:
        ...
        id_docs = sei_client.md_ia_lista_documentos_elegiveis_processos_similares(
            processes_str
        )
    ...
    def get_process_and_subprocesses_str(id_protocolo, subprocesses=True):
        ...
        if subprocesses:
            subproc = sei_client.get_subprocessos_id_protocolo(id_protocolo)
    ...
    def get_process_metadata(id_protocolo):
        df_metadata = sei_client.md_ia_consulta_processo_metadados(
            id_procedimento=id_protocolo
        )
    ...
    def get_docs_from_process(id_protocolo):
        ...
        docs_process = consulta_documentos_com_conteudo(id_docs_str)
    ...
    def agg_related_process_query(id_docs_str: str) -> pd.DataFrame:
        ...
        df_docs = consulta_documentos(id_docs_str)
        records = []
        if df_docs.empty:
            return pd.DataFrame(columns=columns)
        for id_protocolo in df_docs["id_protocolo"].unique():
            df_processo = sei_client.md_ia_consulta_processo_metadados(id_protocolo)
            # md_ia_consulta_processo_metadados devolve um DataFrame com colunas
            # snake_case; pega a 1a linha (campos de processo iguais em todas) e
            # lê por rótulo. Série vazia quando não há metadados preserva os
            # campos como None sem descartar o documento.
            processo = (
                df_processo.iloc[0]
            ...
            for _, row in docs_protocolo.iterrows():
                registros = {
                    ...
                    "protocolo_formatado": processo.get("protocolo_formatado"),
                    "processo_especificacao": processo.get("processo_especificacao"),
                    "interessado": processo.get("interessado"),
                    "name_interested": processo.get("name_interested"),
                    "id_type_process": processo.get("id_type_process"),
                    ...
                    "name_id_type_process": processo.get("name_id_type_process"),
