...
from jobs.db_models.repository import get_app_db
...
class ProcessTransformed(BaseModel):
    ...
    def fabric_solr_dict(self) -> None:
        ...
        self.solr_dict = {
            **metadata_process,
            **documents_content,
            "version_manager_id": int(get_job_version_manager(get_app_db())),
    ...
    def _build_section_fields(self, id_type_doc: str, content: list) -> dict:
        result = {}
        sects_info = SECTIONS_DICTIONARY[self._DOCS_WITH_SECTIONS[str(id_type_doc)]]
        for ith_content in content:
            if ith_content is None:
                continue
            sects = SplitSection(
            ...
            for k in sects_info:
                ...
                if field_name not in result:
                    result[field_name] = [remove_sep_token(sects[k])]
                else:
                    result[field_name].append(remove_sep_token(sects[k]))
        return result
    ...
    def __spread_documents_content(self):
        ...
        for id_type_doc, content in self.content_id_type_docs_aggs.items():
            ...
            if str(id_type_doc) in self._DOCS_WITH_SECTIONS:
                content_spread.update(self._build_section_fields(id_type_doc, content))
