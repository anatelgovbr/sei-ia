...
def drop_canceled_processes_from_solr(
    ...
    while True:
        delete_list, id_ultimo = sei_client.md_ia_lista_processos_indexaveis_cancelados(
        ...
        async def remove_ids_async(ids_to_remove=delete_list) -> None:
            tasks = [
                sei_client.md_ia_remove_processos_indexaveis_cancelados_async(_id)
...
def drop_canceled_documents_and_embeddings(
    ...
    while True:
        delete_list, id_ultimo = (
            sei_client.md_ia_lista_documentos_indexaveis_cancelados(
                batch_size, id_ultimo
            )
        )
        ...
        async def remove_ids_async(ids_to_remove=delete_list) -> None:
            tasks = [
                sei_client.md_ia_remove_documentos_indexaveis_cancelados_async(_id)
                for _id in ids_to_remove
            ]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for i, result in enumerate(results):
...
with DAG(
    ...
    # Task 1 → Task 2: processos primeiro, depois documentos
    task_processes = drop_canceled_processes_from_solr(
        batch_size=INDEX_BATCH_SIZE * 5,
        solr_url=SOLR_ADDRESS,
        solr_core=SOLR_MLT_PROCESS_CORE,
        auth=auth,
    )
    task_documents = drop_canceled_documents_and_embeddings(
        batch_size=INDEX_BATCH_SIZE * 5,
        solr_url=SOLR_ADDRESS,
        solr_core=SOLR_MLT_DOCUMENTS_CORE,
        auth=auth,
    )
    task_processes >> task_documents  # noqa: B015
