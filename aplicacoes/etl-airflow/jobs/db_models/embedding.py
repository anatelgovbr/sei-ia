...
from sqlalchemy import Column, DateTime, Index, Integer, MetaData, func
...
class EmbeddingsTable(BasePgvector):
    ...
    # DDL idempotente e IDÊNTICO ao do assistente (table_manager.py): mesma dimensão
    # de vetor e mesmos índices (id_documento + ivfflat cosine). Garante que, em
    # instalação nova, quem criar a tabela primeiro (ETL via metadata.create_all ou
    # assistente via CREATE TABLE IF NOT EXISTS) produza exatamente a mesma tabela.
    __table_args__ = (
        Index(
            f"idx_{EMBEDDINGS_TABLE_NAME}_id_documento",
            "id_documento",
        ),
        Index(
            f"idx_{EMBEDDINGS_TABLE_NAME}_embedding",
            "embedding",
            postgresql_using="ivfflat",
            postgresql_with={"lists": 100},
            postgresql_ops={"embedding": "vector_cosine_ops"},
        ),
        {"schema": DB_SEIIA_ASSISTENTE_SCHEMA},
    )
