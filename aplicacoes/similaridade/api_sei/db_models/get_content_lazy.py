...
from api_sei.db_models.sei_client import consulta_documentos_com_conteudo, sei_client
...
def get_doc_content_lazy(list_id_docs: list, *, raw: bool = False) -> list[dict]:
    ...
    """
    try:
        # Busca metadados + conteúdo dos documentos
        ids_str = ",".join(map(str, list_id_docs))
        df_docs = consulta_documentos_com_conteudo(ids_str)
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao buscar documentos: {e!s}"
        ) from e
...
def get_proc_content_lazy(id_proc: int) -> dict[str, str | list[str]]:
    ...
    try:
        ...
        df_process_metadata = sei_client.md_ia_consulta_processo_metadados(str(id_proc))
        ...
        try:
            list_doc_ids = (
                sei_client.md_ia_lista_documentos_elegiveis_processos_similares(
                    str(id_proc)
                )
            )
            if not list_doc_ids:
            ...
            df_docs_processo = consulta_documentos_com_conteudo(
                ids_str,
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro interno ao buscar processo: {e!s}"
        ) from e
