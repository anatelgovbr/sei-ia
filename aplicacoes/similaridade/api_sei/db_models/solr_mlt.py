...
class SolrMlt:
    ...
    def _build_like_query(self, solr_doc_id: str) -> str:
        ...
        query = self._build_initial_query(solr_doc_id)
        query = self._add_mlt_filters(query)
        return query + "&mlt.boost=true&wt=json&mlt.interestingTerms=details"
    ...
    def _empty_mlt_response(self) -> dict[str, Any]:
    ...
    def _mlt_request(self, solr_doc_id, rows, pfq, nfq, request_type):
        if request_type == "indirect":
            ...
            if not parsedquery.strip():
                logger.warning(
                    "No interesting terms found for %s; returning an empty recommendation",
                    solr_doc_id,
                )
                return self._empty_mlt_response()
    ...
    def mlt(self, solr_doc_id, rows=10, fq=None):
        ...
        if self.config.normalized and recommendation["recommendation"]:
