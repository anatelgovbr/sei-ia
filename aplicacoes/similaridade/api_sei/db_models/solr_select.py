...
RESPOSTA_INVALIDA_DO_SOLR = "Resposta inválida do Solr"
...
async def async_solr_requests(
    ...
    async def _do_requests() -> list[Any]:
        timeout = httpx.Timeout(
            connect=5.0,
            read=SOLR_READ_TIMEOUT_SECONDS,
            write=10.0,
            pool=5.0,
        )
        async with httpx.AsyncClient(
            auth=auth, verify=VERIFY_SSL, timeout=timeout
        ) as client:
            tasks = [client.get(q) for q in queries]
            return await asyncio.gather(*tasks, return_exceptions=True)

    # asyncio.timeout() só existe no Python 3.11+; o runtime aqui é 3.10,
    # então usamos asyncio.wait_for(), que tem semântica equivalente.
    return await asyncio.wait_for(_do_requests(), timeout=SEI_API_DB_TIMEOUT)
...
class SolrRequests:
    ...
    def retrieve_response(http_response: Any, nested_fields: list | None = None) -> Any:  # noqa: ANN401
        ...
        if isinstance(http_response, Exception):
            logger.error(
                "SolrException: resposta do Solr é uma exceção: %r", http_response
            )
            raise SolrException(
                status_code=503,
                detail="Erro de comunicação com o Solr",
            ) from http_response
        ...
        if not (
            ...
            raise SolrException(
                status_code=500,
                detail=RESPOSTA_INVALIDA_DO_SOLR,
            )
    ...
    def async_select(
        ...
        try:
            response_list = loop.run_until_complete(
                async_solr_requests(queries, auth=auth)
            )
        except asyncio.TimeoutError as exc:
            logger.error("Tempo limite excedido ao consultar o Solr")
            raise SolrException(
                status_code=503,
                detail="Tempo limite ao consultar o Solr",
            ) from exc

        responses = []
        try:
            for query, response in zip(queries, response_list, strict=True):
                if isinstance(response, asyncio.CancelledError):
                    raise response
                if isinstance(response, Exception):
                    logger.error("Falha na consulta ao Solr %s: %r", query, response)
                    raise SolrException(
                        status_code=503,
                        detail="Erro de comunicação com o Solr",
                    ) from response
                responses.append(
                    SolrRequests.retrieve_response(response, nested_fields)
                )
        except (TypeError, ValueError) as exc:
            logger.error("SolrException: quantidade de respostas do Solr inválida")
            raise SolrException(
                status_code=500,
                detail=RESPOSTA_INVALIDA_DO_SOLR,
            ) from exc
