...
def _compute_normalize_value(normalized, parsedquery, found_id_docs, text):
    if not (normalized and parsedquery.strip()):
        return 1
    tokens = []
    ...
    value = get_doc_score(BASE_URL_JURISPRUDENCE, parsedquery, {"content": tokens})
...
def doc2doc_search(
    ...
    list_type_id_doc: list[int] | None,
    id_user: int | None,
    rows: int,
    text: str = "",
    ...
    normalize_value = _compute_normalize_value(
        normalized, parsedquery, found_id_docs, text
    )
