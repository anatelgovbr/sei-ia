...
SOLR_INDEX_VISIBILITY_ATTEMPTS = 5
SOLR_INDEX_VISIBILITY_INTERVAL_SECONDS = 2
SOLR_INDEX_VISIBILITY_REQUEST_TIMEOUT_SECONDS = 2
SOLR_INDEX_VISIBILITY_TIMEOUT_SECONDS = (
    (SOLR_INDEX_VISIBILITY_ATTEMPTS - 1) * SOLR_INDEX_VISIBILITY_INTERVAL_SECONDS
...
def _wait_for_solr_index_visibility(
    nr_process: int, get_indexed_docs: Callable[[float], list]
) -> None:
    started_at = time.monotonic()
    deadline = started_at + SOLR_INDEX_VISIBILITY_TIMEOUT_SECONDS
    checks = 0

    for attempt in range(SOLR_INDEX_VISIBILITY_ATTEMPTS):
        scheduled_at = started_at + attempt * SOLR_INDEX_VISIBILITY_INTERVAL_SECONDS
        ...
        if get_indexed_docs(request_timeout):
            logger.info(
                "Solr made %s visible after %d check(s)",
                nr_process,
                checks,
            )
            return
    ...
    logger.warning(
        "Solr did not make %s visible after %d check(s)",
        nr_process,
        checks,
    )
...
def get_all_str_search_fields(
    ...
    def get_indexed_docs(timeout: float | None = None) -> list:
    ...
    if docs:
        ...
        docs = SolrRequests.get(
            url=f"{JOBS_API_ADDRESS}/process/unindexed/nr_process/{nr_process}",
            rows=1,
            start=0,
            auth=auth,
        )

        fallback_doc = docs[0] if isinstance(docs, list) and docs else None
        if (
            not isinstance(fallback_doc, Mapping)
            or not fallback_doc
            or str(fallback_doc.get("id_protocolo")) != str(nr_process)
        ):
            raise ResourceNotFoundException(resource_name=nr_process)

        _wait_for_solr_index_visibility(nr_process, get_indexed_docs)

        all_docs_fields = list(fallback_doc.keys())
        pos_regex = re.compile(r"|".join(SEARCH_FIELDS_PREFIXES))
        search_fields = list(filter(pos_regex.search, all_docs_fields))
...
def read_mlt_fields_weights(mlt_fields: list) -> dict:
    ...
    conf_mlt_fields_weights = app_db.execute_query_one(
        f"SELECT * FROM {ConfigMltFieldsWeights.__tablename__} ORDER BY id DESC"  # noqa: S608 # NOSONAR
    )
...
class FasterCustomParsedQuery(FasterCustomParsedQueryBaseClass):
    ...
    def _execute_multiple_requests(self, field_group: list, *, parallel: bool) -> list:
        ...
        """
        rows_parameter = "&rows=0" if parallel and len(field_group) > 1 else ""
        queries = [
            (
                f"{self.base_url}/mlt?q=id_protocolo:{self.id_protocolo!s}&fl=id_protocolo,score"
                f"&mlt.maxqt=25&mlt.fl={field_name}"
                f"&mlt.mintf=1&mlt.mindf=1&wt=json&mlt.interestingTerms=details&mlt.boost=true{rows_parameter}"
            )
    ...
    def get_interestingterms(
        self, *, parallel: bool = True, speed_level: int = 2, **_kwargs
    ) -> dict:  # noqa: ANN003
        """Recupera os termos mais interessantes de cada campo de busca de texto.
...
class ManualExtractCustomParsedQuery(FasterCustomParsedQueryBaseClass):
    ...
    def get_interestingterms(self, **kwargs) -> dict:  # noqa: ANN003, ARG002  # NOSONAR
