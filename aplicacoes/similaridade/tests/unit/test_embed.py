...
def mock_db_client() -> any:
    ...
    Example:
        ...
        ```
    """
    with patch("api_sei.resources.embed.app_db") as mock:
        yield mock
...
def test_get_similarity_embedding_no_list_id_processos() -> None:
    """Test case for the `get_similarity_embedding` function when `list_id_processos` is an empty list.

    Verifica que a função levanta ValueError quando `list_id_processos` é uma lista vazia
    (comportamento atual de api_sei/resources/embed.py).
    ...
    with pytest.raises(ValueError) as excinfo:
        get_similarity_embedding(1, [], 2)
    assert str(excinfo.value) == "list_id_processos não pode ser vazio"
...
def test_get_similarity_embedding_invalid_list_id_processos() -> None:
    ...
    This test case verifies that the function raises a `TypeError` when the `list_id_processos` parameter is not a list.
    The expected error message is "list_id_processos deve ser uma lista".
    ...
    assert str(excinfo.value) == "list_id_processos deve ser uma lista"
...
def test_get_similarity_embedding_invalid_id_processo() -> None:
    ...
    This test case verifies that the function raises a `TypeError` when the `id_processo` parameter is not an integer.
