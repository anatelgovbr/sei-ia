...
@patch("api_sei.repository.log_consume.app_db")
...
@patch("api_sei.repository.log_consume.app_db", None)
...
def test_insert_log_consume_exception(mock_db_client, mock_logger, log_data):
    ...
    mock_db_client.add = MagicMock(side_effect=SQLAlchemyError("DB error"))
    ...
    with pytest.raises(SQLAlchemyInsertError, match="DB error"):
        insert_log_consume(log_consume=log_consume)
