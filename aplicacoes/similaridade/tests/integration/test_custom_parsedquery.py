...
from tests.integration.mock_solr import mock_requests_get
...
class TestCustomParsedQuery(unittest.TestCase):
    ...
    @mock.patch("api_sei.resources.custom_parsedquery.app_db")
    @pytest.mark.integration
    def test_get_parsedquery(self, mock_app_db):
        # Config de pesos hoje vem do banco (ConfigMltFieldsWeights), nao mais de
        # um JSON estatico. O peso do bucket "metadata" (raiz) nao chega a ser
        # aplicado pelo algoritmo de read_weight para campos com um unico nivel
        # de aninhamento (metadata.fields.<campo>) -- so o peso do proprio campo
        # conta. Verificado empiricamente com read_mlt_fields_weights().
        field_weight = 0.5
        config = {
            "metadata": {
                "weight": 0.3,
                "fields": {
                    "metadata_name_id_type_process": {
                        "weight": field_weight,
                        "variable_subfields": None,
                    },
                },
            },
            "content": {"weight": 0.7, "fields": {}},
        }
        mock_app_db.execute_query_one.return_value = (1, config)
        ...
        expected_weights = [field_weight/self_score]*len(metadata_name_id_type_process_query_terms)
    ...
    def test_process_text_similarity_field_small_weight(self):
        ...
        self.assertNotIn("e-", parsedquery)
