...
from tests.integration.mock_solr import mock_requests_get, mock_requests_post
...
def test_doc2doc_search_no_parsedquery(mock_get_tokenized_docs,list_id_doc):
    ...
    if list_id_doc:
        # doc nao encontrado no Solr + sem texto -> doc2doc_search captura a
        # ParsedQueryEmptyException internamente e retorna lista vazia (200),
        # nao propaga como erro.
        assert response.status_code == 200
        assert response.json()["recommendation"] == []
    else:
        assert response.status_code == 400
        assert response.json()["detail"] == "list_id_doc ou text não podem ser ambos vazios"
