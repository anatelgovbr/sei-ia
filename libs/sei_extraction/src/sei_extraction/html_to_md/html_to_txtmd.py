...
class HtmlTxtmd:
    ...
    def navigate_el(self, node, counters, levels, outputs):  # NOSONAR  # noqa: C901
        ...
        # self.logger.debug(f"{identtion}[{outputs=}]")  # noqa: ERA001
    ...
    def process_table_caption(
        self,
        table,
        _counters,  # noqa: ARG002  # NOSONAR
        levels,
        outputs,
    ) -> str:
    ...
    def process_hr(self, child, counters, levels, outputs):  # noqa: ARG002  # NOSONAR
        self.outputs_append(outputs, "---\n\n")

    def process_pre(self, child, counters, levels, outputs):  # noqa: ARG002  # NOSONAR
    ...
    def navigate_phrasing(self, node, levels):  # noqa: C901, PLR0912, PLR0915  # NOSONAR
        chunks = []
        for child in node:
            output = None
            if child.name in ["b", "strong"]:
                ...
                if href:
                    ...
                    chunks.append(f"[{output}]({href}{title_str})")
                else:
